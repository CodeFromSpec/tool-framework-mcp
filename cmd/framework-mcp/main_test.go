...
func TestToolsListAdvertisesAllTools(t *testing.T) {
	...
	expected := []string{"load_chain", "write_artifact", "write_verdict", "validate_specs", "accept", "create_token", "dump_chain", "reconstruct_cache", "prune_cache", "prune_orphans", "version"}
	found := make(map[string]bool)
	for _, tool := range tools {
		toolMap, ok := tool.(map[string]any)
		if !ok {

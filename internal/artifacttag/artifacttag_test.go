// code-from-spec: ROOT/golang/tests/parsing/artifact_tag@cvD6hF0SlmXslBifWsPiEGO7X90
...
func TestArtifactTagExtract_HashComment(t *testing.T) {
	...
	if err := os.WriteFile("file.py", []byte(content), 0600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
func TestArtifactTagExtract_HTMLComment(t *testing.T) {
	...
	if err := os.WriteFile("file.html", []byte(content), 0600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
func TestArtifactTagExtract_EmptyFile(t *testing.T) {
	...
	if err := os.WriteFile("file.go", []byte(""), 0600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}

...
- `ARTIFACT/x/y(id)` — imports the content of the referenced
  artifact, excluding any frontmatter, as context (not as
  material to transform; see `input` for that).
...
excluding any frontmatter. While `depends_on` brings in context
that informs generation, `input` brings in content that the
generation subagent transforms into a new artifact.

```yaml
---
input: ARTIFACT/functional/notifications(logic)
---
```

#### outputs

Artifacts generated by this node. Each entry has an `id` and
a `path` (relative to the project root, forward slashes).
...
6. If the target node has an `input` field, the content of the
   referenced artifact, excluding any frontmatter, is included
   as the input to transform.
...
ARTIFACT/functional/fees/calculation(calc) [full]    ← input
```

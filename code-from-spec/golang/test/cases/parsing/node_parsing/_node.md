...
# SPEC/golang/test/cases/parsing/node_parsing
...
### Every fixture needs the node name heading

Every test fixture must start with
`# SPEC/<name>` as its first `#` heading, even when
the test focuses on a different section. `ParseNode`
rejects files without it. When a test case says
"Node with `# PUBLIC` heading", the file must have
`# SPEC/<name>` followed by `# PUBLIC` — not
`# PUBLIC` alone.

### Counting content lines in fixtures

Content is the list of lines between one structural
heading and the next (or end of file). Count every
line, including:
- Blank lines (preserved as empty strings)
- Fenced code block opening fences (e.g. ` ```go `)
- Lines inside fenced code blocks
- Fenced code block closing fences (e.g. ` ``` `)
- Level-3+ headings (`###`, `####`, etc.)

The closing fence of a code block is a content line
just like the opening fence. Do not omit it from the
expected count.

When writing the expected line count in a test, build
the fixture string first, then count the lines between
the subsection heading and the next structural heading
to derive the expected number.
...
Expected: ErrUnexpectedContentBeforeFirstHeading —
the line is not recognized as a frontmatter delimiter,
so the entire content is body, which starts with
non-heading content.
...
Expected:
- Content has 5 lines: the level-3 heading, the bold
  text, the opening fence, the code line, and the
  closing fence. All preserved as raw strings.

...
# ROOT/golang/internal/chain_resolver/code
...
Read the target node's frontmatter using `ParseFrontmatter`
and extract the `Outputs` list. For each output in `Outputs`,
use the `Path` field and check if the file exists on disk
(using `os.Stat`). If it exists, add the path to `Code`. If
it does not exist, skip it. `Code` contains only files that
already exist.
